...
$return = in_car_process($car_num, $garage_id, $result6, $result7, $if_car_num, $if_order, $if_late);
$in_cp_num = $return['in_cp_num'];
$if_car_num = $return['if_car_num'];
$if_order = $return['if_order'];
$if_late = $return['if_late'];
/******************END*******************/
...
$back = back_to_danpianji($confirm_parking, $in_cp_num, $out_cp_num, $device_id, $floor_1_id,
    $if_order, $if_out, $give_money, $if_car_num, $if_late);
...
//        echo '<br>';
    ...
    for ($i = 0; $i < strlen($floor_1_status); $i++) {
        if ($place[$i] == "1") {//限位开关为1，表示当前位置是第一层的车位
            ...
            if ($status[$i] != $cp_k_status && $cp_k_status == 0) {//如果单片机传来的当前位置信息与数据库不匹配，则表示车停到了该车位
                $get_cp_num = 1;
                $true_cp_num = $k;
            }
            $k++;
...
//                   echo $j;
    ...
    if ($get_cp_num == 0) {//若没有停在第一层，则停在了高层车架上
        ...
        if ($status[$j] != $cp_high_status && $cp_high_status == 0) {
...
//存入单片机数据表
    if(!mysql_query("update danpianji set time1 = '$time',device_id = '$device_id',car_num = '$car_num',
...
//存车流程函数
function in_car_process($car_num, $garage_id, $result6, $result7, $if_car_num,$if_order, $if_late)
{
    $in_cp_num = "0";

    if ($car_num != "0") {
        $if_car_num = "1";
        $sql_12 = mysql_query("select * from late_order where car_num = '$car_num'");
...
//        echo $result4['car_num'];
    ...
    $return['if_car_num'] = $if_car_num;
    $return['if_order'] = $if_order;
    $return['if_late'] = $if_late;
    $return['in_cp_num'] = $in_cp_num;
    return $return;
}
...
//    echo $out_cp_num;
        if ($out_cp_num != "0") {
            $if_out = "1";
            $sql_x = mysql_query("select * from parking_info where pay_status = 1 and confirm_out = 0");
            $result_x = mysql_fetch_assoc($sql_x);
            $give_money = $result_x['money'];
...
//    echo("if_car_num:".$if_car_num);
//    echo '<br>';
//    echo("if_order:".$if_order);
//    echo '<br>';
//    echo("if_late:".$if_late);
//    echo '<br>';
//    echo("if_out:".$if_out);
//    echo '<br>';

    if($if_car_num == "1" && $if_order == "1" && $if_late == "0" && $if_out == "0"){
        $led = "1";//已扫描到车牌，已预约
    }else if($if_car_num == "1" && $if_late == "1" && $if_out == "0"){
        $led = "2";//已扫描到车牌，超时
    }else if($if_car_num == "1" && $if_order == "0" && $if_out == "0"){
        $led = "3";//已扫描到车牌，未预约
    }else if($if_out == "1"){
        $led = "4";//取车操作
    }else{
    ...
    $back['led'] = $led;
    $back['money'] = (string)$give_money;
//    echo("money:".$back['money']);
//    echo '<br>';
    return $back;
}
